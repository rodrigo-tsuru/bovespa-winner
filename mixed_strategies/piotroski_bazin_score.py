...
def populate_shares(year):
  ...
  shares['Score'] = shares['Piotroski Score'] + shares['Bazin Score']
...
def reorder_columns(shares):
  columns = ['Ranking', 'Score', 'Cotação', 'Preço Justo (Bazin)', 'Preço Justo (Bazin) / Cotação', 'Piotroski Score', 'Bazin Score', 'Media de Dividend Yield dos Últimos 5 anos', 'Dividend Yield']
...
if __name__ == '__main__':
  ...
  shares.sort_values(by=['Score', 'Preço Justo (Bazin) / Cotação'], ascending=[False, False], inplace=True)
